...
var PDFPageView = (function PDFPageViewClosure() {
  ...
  PDFPageView.prototype = {
    ...
    draw: function PDFPageView_draw() {
      ...
      //自己改动
        var waterBox = document.createElement('div');
        waterBox.setAttribute("class", "waterBox");
        var ul = document.createElement("ul");


        for(var i=0;i<9;i++){
            var li = document.createElement("li");
            li.setAttribute("class", "waterLi");
            var textnode=document.createTextNode(username);
            li.appendChild(textnode);
            ul.appendChild(li);
        }

        waterBox.appendChild(ul);
        waterBox.style.width = div.style.width;
        waterBox.style.height = div.style.height;
      canvas.id = 'page' + this.id;
      canvasWrapper.appendChild(canvas);

        canvasWrapper.appendChild(waterBox);
      if (this.annotationLayer) {
