...
(function (factory) {
    ...
    defaultPreviewZoomSettings = {
        ...
        object: {width: "auto", height: "100%", 'min-height': "100px"},
    ...
    findFileName = function (filePath) {
        ...
        console.log(filePath.split(filePath.substring(sepIndex, sepIndex + 1)).pop());
    ...
    FileInput.prototype = {
        ...
        _initSuccessThumbs: function () {
            ...
            self._getThumbs('.file-preview-success').each(function () {
                ...
                handler($remove, 'click', function () {
                  //@TODO BMW项目修改的源码 2017_11_21

                    var out = self._raise('filesuccessremove', [$thumb,$thumb.attr('id'), $thumb.data('fileindex')]);
        ...
        _uploadExtra: function (previewId, index) {
            ...
            //@Modify MTY BMW
            $.each(GLOBAL_MEETING_FILES, function (key, data) {
                self.formdata.append(GLOBAL_FILE_NAME, GLOBAL_MEETING_FILES[key].data, GLOBAL_MEETING_FILES[key].name);
            });
        },
        ...
        _uploadBatch: function () {
            ...
            $.each(files, function (key, data) {
                if (!isEmpty(files[key])) {
                    // console.log(1994)
                    // console.log(self.filenames[key])
                    // console.log(data)
                    // console.log(self.uploadFileAttr)
        ...
        _previewFile: function (i, file, theFile, previewId, data) {
            ...
            $('#bmwDetail .editImg>img').attr('src',iData);
        ...
        _readFiles: function (files) {
            ...
            readFile = function (i) {
                ...
                /**
                 * @TODO@TODO验证所上传文件名是否符合规范
                 */


                // var fileReg = /^(\d{4}) (MINI|BMW) SPQ Project Q([1-4]) Dealer Quarterly Report_(\d+)[^\.]+\.xlsx$/;
                // var fileReg = /^(\d{4}) (MINI|BMW) SPQ Project Q([1-4]) Dealer Quarterly Report_(\d+)[^\.]+$/;
                // var suffix=caption.substring(0,caption.lastIndexOf("."));
                // // console.log(fileReg.test(caption))
                // if(!fileReg.test(suffix)){
                //
                //     msg = self.msgFileReg.replace('{name}', caption).replace('{size}', fSizeKB)
                //     self.isError = throwError(msg, file, previewId, i);
                //
                //     return ;
                // }else{
                //     var brand = $('#switchDateType .btn-active').text();
                //     var date = $('#stateSele option:selected').text();
                //     if(date==''){
                //         msg = self.msgFileDateNull;
                //         self.isError = throwError(msg, file, previewId, i);
                //         return;
                //     }
                //     // var update = $.trim(caption.substring(0,4))+' '+$.trim(caption.substring(21,24));
                //     // var upbrand = $.trim(caption.substring(5,9));
                //     //
                //     // if(upbrand!=brand){
                //     //     msg = self.msgFileBrand.replace('{name}', caption).replace('{size}', fSizeKB)
                //     //     self.isError = throwError(msg, file, previewId, i);
                //     //     return ;
                //     // }else if(date!=update){
                //     //     msg = self.msgFileDate.replace('{name}', caption).replace('{size}', fSizeKB)
                //     //     self.isError = throwError(msg, file, previewId, i);
                //     //     return ;
                //     // }
                //
    ...
    $.fn.fileinput.defaults = {
        ...
        minFileSize: 0,
        maxFileSize: 0,
        maxFilePreviewSize: 0, // 25 MB
        // maxFilePreviewSize: 10000000000000, // 25 MB
        minFileCount: 0,
        maxFileCount: 600,
